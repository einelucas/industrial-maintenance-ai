...
import { IncidentProcessStepper, type ProcessStep } from "@/features/thermal-incidents/components/incident-process-stepper";
...
export default async function ThermalIncidentDetailPage({ params }: { params: { id: string } }) {
  ...
  const predictionForReview = latest ?? incident.triggerPrediction;
  const reviewPriorityContext = await thermalPriorityPolicyService.resolveForReview({
    riskLevel: predictionForReview.riskLevel,
    recommendedCompanyPriority: predictionForReview.recommendedCompanyPriority ?? incident.recommendedCompanyPriority,
    priorityPolicyVersion: predictionForReview.priorityPolicyVersion ?? incident.priorityPolicyVersion,
  });
  ...
  const orderBlocked = workOrderBlockReason({ aiStatus: ai.status, canConvert: can(user.role, "incident:convert-to-work-order"), status: incident.status, decision: incident.humanReviewDecision, workOrderId: incident.workOrderId, equipmentId: incident.thermalPoint.component.panel.equipmentId, validEvidence: originValid, finalCompanyPriority: incident.finalCompanyPriority, priorityPolicyVersion: incident.priorityPolicyVersion });
  const historicalPriority = incident.thermalPoint.inspectionFindings[0]?.companyPriority ?? null;
  const reviewDone = incident.humanReviewDecision !== null;
  const processSteps: ProcessStep[] = [
    { key: "inspection", label: "Inspeção histórica", status: historicalPriority ? "completed" : "unavailable", note: historicalPriority ? `Achado original: ${historicalPriority}` : "Ponto fora dos 19 achados originais" },
    { key: "reading", label: "Leitura térmica", status: incident.triggerPrediction.thermalReading ? "completed" : "blocked", note: incident.triggerPrediction.thermalReading ? formatDateTime(incident.triggerPrediction.thermalReading.measuredAt) : "Leitura de origem indisponível" },
    { key: "inference", label: "Inferência da IA", status: originValid ? "completed" : "blocked", note: originValid ? undefined : "Evidência sem proveniência rastreável" },
    { key: "incident", label: "Incidente", status: "completed", note: INCIDENT_STATUS_LABELS[incident.status] },
    { key: "review", label: "Revisão humana", status: reviewDone ? "completed" : reviewBlocked ? "blocked" : "current", note: reviewDone ? `Decisão: ${incident.humanReviewDecision}` : reviewBlocked ?? "Aguardando decisão humana" },
    { key: "workorder", label: "Ordem de Serviço", status: incident.workOrder ? "completed" : !reviewDone ? "pending" : orderBlocked ? "blocked" : "current", note: incident.workOrder ? incident.workOrder.number : orderBlocked ?? "Pronta para autorização" },
    { key: "post-action", label: "Monitoramento pós-ação", status: incident.normalizedAt ? "completed" : incident.workOrder?.status === "COMPLETED" ? "current" : incident.workOrder ? "pending" : "unavailable", note: incident.normalizedAt ? "Normalização registrada" : undefined },
  ];
  const timeline = [
    ...
    ...incident.humanReviews.map((r) => ({ id: r.id, at: r.createdAt, title: `${r.reviewedBy.name} · ${r.decision}`, description: `${r.justification ?? "Sem justificativa adicional"} · ${INCIDENT_STATUS_LABELS[r.previousStatus]} → ${INCIDENT_STATUS_LABELS[r.nextStatus]} · Prioridade: ${r.finalCompanyPriority ?? "não confirmada"} · Política: ${r.priorityPolicyVersion ?? "—"} · Prediction revisada: ${r.reviewedPredictionId}` })),
  ...
  return <div className="space-y-6"><Breadcrumbs items={[{ label: "Incidentes térmicos", href: "/thermal-incidents" }, { label: incident.thermalPoint.code }]} />
    ...
    <IncidentProcessStepper steps={processSteps} />
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <Card><CardHeader><CardTitle>Pico registrado</CardTitle></CardHeader><CardContent>{numeric(incident.peakTemperatureC, " °C")}</CardContent></Card>
      <Card><CardHeader><CardTitle>Pico de ΔT</CardTitle></CardHeader><CardContent>{numeric(incident.peakDeltaTC, " °C")}</CardContent></Card>
      <Card><CardHeader><CardTitle>Último score registrado</CardTitle></CardHeader><CardContent>{originValid ? numeric(incident.lastRiskScore) : "Indisponível"}</CardContent></Card>
      <Card><CardHeader><CardTitle>Prioridade empresarial</CardTitle></CardHeader><CardContent>
        {incident.finalCompanyPriority ? <CompanyPriorityBadge priority={incident.finalCompanyPriority} prefix="Final" />
          : incident.recommendedCompanyPriority ? <CompanyPriorityBadge priority={incident.recommendedCompanyPriority} prefix="Recomendada" />
          : "Sem recomendação"}
      </CardContent></Card>
    </div>
    ...
    <div className="grid gap-4 xl:grid-cols-2"><Card><CardHeader><CardTitle>Decisão humana sobre o defeito</CardTitle></CardHeader><CardContent>{can(user.role, "incident:diagnose") ? <HumanReviewForm incidentId={incident.id} predictionId={latest?.id ?? incident.triggerPredictionId} recommendedPriority={reviewPriorityContext.priority} policyVersion={reviewPriorityContext.policyVersion} blockedReason={reviewBlocked} /> : <p className="text-sm">Seu perfil permite consultar as decisões registradas.</p>}</CardContent></Card><Card><CardHeader><CardTitle>Autorizar manutenção preditiva</CardTitle></CardHeader><CardContent>{incident.workOrder ? <Link className="inline-flex items-center gap-2 text-primary underline" href={`/work-orders/${incident.workOrder.id}`}>{incident.workOrder.number} <WorkOrderStatusBadge status={incident.workOrder.status} /></Link> : <PredictiveWorkOrderForm incidentId={incident.id} blockedReason={orderBlocked} />}</CardContent></Card></div>
    <Card><CardHeader><CardTitle>Evolução e monitoramento pós-ação</CardTitle></CardHeader><CardContent className="space-y-3"><p className="text-sm">{incident.workOrder?.status === "COMPLETED" ? "OS concluída. Consulte as leituras posteriores; a conclusão da OS não comprova normalização térmica." : "Aguardando intervenção concluída para avaliar o período pós-ação."}</p>
      ...
      <ThermalHistoryChart absoluteLimitC={config.values.absoluteLimitC} readings={[...readings].reverse().map((r) => ({ id: r.id, timestamp: r.measuredAt.getTime(), temperatureMaxC: r.temperatureMaxC, referenceTemperatureC: r.referenceTemperatureC, deltaTC: r.deltaTC, currentA: r.currentA, loadPercent: r.loadPercent }))} />
    </CardContent></Card>
