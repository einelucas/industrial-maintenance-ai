...
export default async function ThermalIncidentDetailPage({ params }: { params: { id: string } }) {
  ...
  return <div className="space-y-6"><Breadcrumbs items={[{ label: "Incidentes térmicos", href: "/thermal-incidents" }, { label: incident.thermalPoint.code }]} />
    ...
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{[["Pico registrado", numeric(incident.peakTemperatureC, " °C")], ["Pico de ΔT", numeric(incident.peakDeltaTC, " °C")], ["Último score registrado", originValid ? numeric(incident.lastRiskScore) : "Indisponível"], ["Prioridade empresarial", incident.finalCompanyPriority ? COMPANY_PRIORITY_LABELS[incident.finalCompanyPriority] : incident.recommendedCompanyPriority ? `${COMPANY_PRIORITY_LABELS[incident.recommendedCompanyPriority]} (recomendada)` : "Sem recomendação"]].map(([title, value]) => <Card key={title}><CardHeader><CardTitle>{title}</CardTitle></CardHeader><CardContent>{value}</CardContent></Card>)}</div>
