...
export interface ThermalChartReading {
  id: string; time: string; temperatureMaxC: number; referenceTemperatureC: number | null;
  ...
  return <div className="space-y-5">
    <div className="h-72 w-full" role="img" aria-label="Evolução de temperatura, referência e delta T. Valores também disponíveis na tabela de leituras.">
      <ResponsiveContainer width="100%" height="100%"><LineChart data={readings} margin={{ top: 20, right: 20, bottom: 5, left: 0 }} accessibilityLayer>
        <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="time" minTickGap={60} tick={{ fontSize: 10 }} /><YAxis unit=" °C" width={65} />
        <Tooltip /><Legend /><ReferenceLine y={absoluteLimitC} stroke="#dc2626" strokeDasharray="6 4" label={{ value: "Limite de engenharia", position: "insideTopRight", fontSize: 11 }} ifOverflow="extendDomain" />
    ...
    <div className="h-60 w-full" role="img" aria-label="Corrente e carga no mesmo intervalo das temperaturas, com escalas independentes.">
      <ResponsiveContainer width="100%" height="100%"><LineChart data={readings} margin={{ top: 10, right: 15, bottom: 5, left: 0 }} accessibilityLayer>
        <CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="time" minTickGap={60} tick={{ fontSize: 10 }} /><YAxis yAxisId="current" unit=" A" width={65} /><YAxis yAxisId="load" orientation="right" unit="%" />
        <Tooltip /><Legend /><Line yAxisId="current" name="Corrente (A)" dataKey="currentA" stroke="#0891b2" dot={false} isAnimationActive={false} /><Line yAxisId="load" name="Carga (%)" dataKey="loadPercent" stroke="#7c3aed" strokeDasharray="5 3" dot={false} isAnimationActive={false} />
      </LineChart></ResponsiveContainer>
    </div>
    <p className="text-xs text-muted-foreground">Limites são contexto de engenharia; não produzem diagnóstico. Lacunas representam campos ausentes.</p>
  </div>;
}
